...
func (e *Executor) Run(ctx context.Context, calls ...taskfile.Call) error {
	...
	for _, call := range calls {
		...
		if err != nil {
			e.ListTasks(FilterInternal(), FilterNoDesc())
			return err
		}
...
// FilterNoDesc removes all tasks that do not contain a description.
func FilterNoDesc() FilterFunc {
	return Filter(func(task *taskfile.Task) bool {
		return task.Desc == ""
	})
}

// FilterInternal removes all tasks that are marked as internal.
func FilterInternal() FilterFunc {
	return Filter(func(task *taskfile.Task) bool {
